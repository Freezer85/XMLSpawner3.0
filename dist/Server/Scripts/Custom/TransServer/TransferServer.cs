	...
	public class TransferServer
	{
		...
		private static void LogServer(string message)
		{
			try
			{
				...
				string logFile = Path.Combine(logDir, "transferserver.log");
				Directory.CreateDirectory(logDir);
				File.AppendAllText(logFile, string.Format("{0} [INFO]: {1}{2}", DateTime.Now, message, Environment.NewLine));
			}
			catch
			{
			}
		...
		{
			CommandSystem.Register("XTS", AccessLevel.GameMaster, new CommandEventHandler(XTS_OnCommand));
		...
		public static void XTS_OnCommand( CommandEventArgs e )
		{
			...
			if( e.Arguments.Length > 0 )
			{
				...
				if(e.Arguments[0].ToLower() == "auth" && e.Arguments.Length > 1)
				{
					...
					if(e.Arguments[0].ToLower() == "list" && e.Mobile != null && e.Mobile.AccessLevel >= AccessLevel.GameMaster)
				{
					...
					if(e.Arguments[0].ToLower() == "start" && e.Mobile != null && e.Mobile.AccessLevel >= AccessLevel.GameMaster)
				{
					if(ServerActive)
					{
					...
					if(e.Arguments[0].ToLower() == "stop" && e.Mobile != null && e.Mobile.AccessLevel >= AccessLevel.GameMaster)
				{
		...
		{
			// default accesslevel is gamemaster
			AccessLevel level = AccessLevel.GameMaster;
		...
		public static byte[] RemoteMessaging_ReceiveMessage( string typeName, byte[] data, out string answerType )
		{
			...
			// Fallback: try to find the type in any loaded assembly if Type.GetType fails.
			if (type == null)
			{
			...
			if ( type != null )
			{
				LogServer(string.Format("ReceiveMessage start: typeName={0}, requestBytes={1}", typeName, data != null ? data.Length : 0));
			...
			{
				// Could not locate the message type; return null so caller gets a null response.
				Console.WriteLine("RemoteMessaging_ReceiveMessage: Unknown incoming type '" + typeName + "'");
				outMsg = null;
			}
			...
			if ( outMsg != null )
			{
				...
				if (outMsg is ErrorMessage)
				{
					LogServer(string.Format("ReceiveMessage error payload: requestType={0}, errorMessage={1}", typeName, ((ErrorMessage)outMsg).Message ?? "<null>"));
				}
				byte[] result = outMsg.Compress();
				LogServer(string.Format("ReceiveMessage response: requestType={0}, responseType={1}, responseBytes={2}", typeName, answerType, result != null ? result.Length : 0));

				return result;
			}
			else
			{
				LogServer(string.Format("ReceiveMessage response: requestType={0}, responseType=<null>, responseBytes=0", typeName));
				answerType = null;
				return null;
			}
		}
